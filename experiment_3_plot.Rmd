...
svg('fig_b.svg', width = 3.5, height = 4)
...
ggplot(comb_b, aes(x = Score, y = as.factor(predictor), fill = as.factor(predictor))) +
  ...
  theme_minimal() +
  labs(title = "B",
       x=expression(phi[~beta[x~sigma]] - phi[~beta[y~sigma]]),
       y = "Lambda") +
  ylab(NULL) +
  theme(legend.position = "none") +
...
ggplot(comb_c, aes(x = Score, y = as.factor(predictor), fill = as.factor(predictor))) +
  ...
  theme_minimal() +
  labs(title = "C",
       x = expression(paste("Median ", phi[~beta[x~sigma]], "- Median", phi[~beta[y~sigma]])),
       y = "Lambda") +
  ylab(NULL) +
  theme(legend.position = "none") +
...
svg('fig_c_adj.svg', width = 3, height = 4)
